...
const AGENT_TYPES = [
   {
      ...
      icon: "FileSearch",
      description: "Scans smart contracts for red flags",
   },
   {
      id: "sentiment",
      label: "Sentiment Agent",
      icon: "MessageSquare",
      description: "Monitors Twitter/X, Telegram, Discord",
   },
   {
      id: "risk-scorer",
      label: "Risk Scorer",
      icon: "AlertTriangle",
      description: "Produces a Low / Medium / High risk score",
   },
   {
      id: "alert",
      label: "Alert Agent",
      icon: "Bell",
      description: "Sends reports to Telegram, Discord, or email",
   },
   {
      id: "wallet-tracker",
      label: "Wallet Tracker",
      icon: "Wallet",
      description: "Tracks on-chain activity for wallets",
   },
   {
      id: "price-monitor",
      label: "Price Monitor",
      icon: "TrendingUp",
      description: "Watches token price and triggers on thresholds",
   },
   {
      id: "liquidity-watcher",
      label: "Liquidity Watcher",
      icon: "Droplets",
      description: "Monitors DEX liquidity pools for rug patterns",
   },
   {
      id: "nft-monitor",
      label: "NFT Monitor",
      icon: "Image",
      description: "Tracks NFT mints, sales, and floor price",
   },
   {
      id: "summarizer",
      label: "Summarizer",
      icon: "FileText",
      description: "Condenses research into a clean report",
   },
   {
      id: "webhook",
      label: "Webhook Agent",
      icon: "Webhook",
      description: "Sends data to any external endpoint",
   },
];
...
export function AddAgentSheet() {
   ...
   return (
      ...
      >
         <SheetContent
            side="right"
            className="w-72 bg-ink-dark border-sand/15 p-0 flex flex-col"
         >
            ...
            <div className="flex-1 overflow-y-auto py-2 px-2 flex flex-col gap-0.5">
               {filtered.length === 0 && (
                  <p className="text-xs text-sand/30 text-center py-8">
                     No agents found
                  </p>
               )}
               {filtered.map((agent) => {
                  ...
                  return (
                     ...
                     >
                        ...
                        <div className="flex flex-col gap-0.5 min-w-0">
                           <span className="text-xs font-medium text-sand leading-none">
                              {agent.label}
                           </span>
                           <span className="text-[10px] text-sand/40 leading-snug truncate">
                              {agent.description}
                           </span>
                        </div>
                     </button>
